    ...
    def parse(self):
        ...
        for line in self.log_file_obj:
            ...
            try:
                line = line.split('|')
                # print(line)
                time_clock = line[0].split(',')[0].strip()

                if 'status' in line[1]:
                    char = line[1].split(':')[2].split()[0].replace('\'','"').strip()
                    # print(char)
                    if char == '"\\177",':#回退
                        char = '[1<-del]'
...
#     parser = AuditLogHandler(os.path.join(AUDIT_LOG_DIR,"ssh_192.168.137.170_root_20210104160646_admin@qq.com.log"))
#     parser.parse()